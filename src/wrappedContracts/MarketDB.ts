    ...
    export class MarketDB extends GeneralFunctions
{
	...
	async setMatcherPropertiesAndURL(_agreementId:number,_matcherPropertiesDocumentHash:string,_matcherDBURL:string, txParams?:SpecialTx){

            let transactionParams

            const txData = await this.web3Contract.methods.setMatcherPropertiesAndURL(_agreementId,_matcherPropertiesDocumentHash,_matcherDBURL)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.setMatcherPropertiesAndURL(_agreementId,_matcherPropertiesDocumentHash,_matcherDBURL)
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.setMatcherPropertiesAndURL(_agreementId,_matcherPropertiesDocumentHash,_matcherDBURL)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
	async createSupply(_propertiesDocumentHash:string,_documentDBURL:string,_assetId:number, txParams?:SpecialTx){

            let transactionParams

            const txData = await this.web3Contract.methods.createSupply(_propertiesDocumentHash,_documentDBURL,_assetId)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.createSupply(_propertiesDocumentHash,_documentDBURL,_assetId)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.createSupply(_propertiesDocumentHash,_documentDBURL,_assetId)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
	async setAgreementMatcher(_agreementId:number,_matchers:string[], txParams?:SpecialTx){
            ...
            const txData = await this.web3Contract.methods.setAgreementMatcher(_agreementId,_matchers)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.setAgreementMatcher(_agreementId,_matchers)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.setAgreementMatcher(_agreementId,_matchers)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
	...
	async createDemand(_propertiesDocumentHash:string,_documentDBURL:string,_demandOwner:string, txParams?:SpecialTx){

            let transactionParams

            const txData = await this.web3Contract.methods.createDemand(_propertiesDocumentHash,_documentDBURL,_demandOwner)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.createDemand(_propertiesDocumentHash,_documentDBURL,_demandOwner)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.createDemand(_propertiesDocumentHash,_documentDBURL,_demandOwner)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
	async approveAgreementSupplyDB(_agreementId:number, txParams?:SpecialTx){
            ...
            const txData = await this.web3Contract.methods.approveAgreementSupplyDB(_agreementId)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.approveAgreementSupplyDB(_agreementId)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.approveAgreementSupplyDB(_agreementId)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
	...
	async approveAgreementDemandDB(_agreementId:number, txParams?:SpecialTx){
            ...
            const txData = await this.web3Contract.methods.approveAgreementDemandDB(_agreementId)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.approveAgreementDemandDB(_agreementId)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.approveAgreementDemandDB(_agreementId)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
	async createAgreementDB(_propertiesDocumentHash:string,_documentDBURL:string,_matcherPropertiesDocumentHash:string,_matcherDBURL:string,_demandId:number,_supplyId:number, txParams?:SpecialTx){

            let transactionParams

            const txData = await this.web3Contract.methods.createAgreementDB(_propertiesDocumentHash,_documentDBURL,_matcherPropertiesDocumentHash,_matcherDBURL,_demandId,_supplyId)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.createAgreementDB(_propertiesDocumentHash,_documentDBURL,_matcherPropertiesDocumentHash,_matcherDBURL,_demandId,_supplyId)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.createAgreementDB(_propertiesDocumentHash,_documentDBURL,_matcherPropertiesDocumentHash,_matcherDBURL,_demandId,_supplyId)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
