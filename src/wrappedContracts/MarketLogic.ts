    ...
    export class MarketLogic extends GeneralFunctions
{
	...
	async createDemand(_propertiesDocumentHash:string,_documentDBURL:string, txParams?:SpecialTx){

            let transactionParams

            const txData = await this.web3Contract.methods.createDemand(_propertiesDocumentHash,_documentDBURL)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.createDemand(_propertiesDocumentHash,_documentDBURL)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.createDemand(_propertiesDocumentHash,_documentDBURL)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
